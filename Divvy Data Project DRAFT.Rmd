...
Manipulation using raw data with nulls to visualize the distributions of no outlier data before and after removing null records, since 1.14M records dropped. The order in which you remove nulls or outliers matters. By removing outliers first, the data frame ***rm_w\_null_wo_outlier*** has a wider range (up to 8.5 hours) compared with having dropped the nulls before removing outliers.
...
Read in the data to remove values past 99 hours since obvious errors and can't be shown in HH:MM:SS format
```{r include=FALSE}
...
threshold <- 0.997 # Somewhat arbitrarily determined

filtered_member <- raw_master %>%
  filter(member_casual == 'member') %>%
  filter(trip_length_sec < quantile(trip_length_sec, threshold)) #7801 records removed


filtered_casual <- raw_master %>%
  filter(member_casual == 'casual')%>%
  filter(trip_length_sec < quantile(trip_length_sec, threshold)) #6047 records removed

rm_w_null_wo_outlier <- bind_rows(filtered_casual, filtered_member) #(combined 13848 records removed as outliers)
...
# Clean Master Data set. No records below 60 seconds, nulls, or greater than 99.7 of data in their category
...
*wo outlier and w null* is equal to *w null wo outlier* because the nulls remain in both cases. It's saying the same thing in a different way. 1) Remove all data points greater than 99.7 percent of the data and leave the remaining nulls in the data (allows for nulls to be removed as outliers) vs 2) leave the nulls in the data and remove 99.7 percent of the data (allows for nulls to be removed as outliers). In both cases, the data will null values were always able to be removed (and either were or weren't, but done the same).
...
```{r}
# Compare the means. Interesting as I'd have expected members to be on longer rides so they avoid higher fees
dtd_member <- dtd %>%
  filter(member_casual == 'member')
